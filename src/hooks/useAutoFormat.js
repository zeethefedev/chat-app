...
export function useAutoFormat() {
  const autoFormatPatterns = [
    ...
    {
      pattern: /\[([^\]]+)\]\(([^)]+)\)/g,
      transform: (_, text, url) => `<a href="${url}" target="_blank" rel="noopener noreferrer" class="text-blue-500 hover:underline">${text}</a>`
    },
    ...
    {
      pattern: /^>\s(.+)$/gm,
      transform: (_, text) => `<blockquote class="border-l-4 border-gray-300 dark:border-gray-600 pl-2 italic">${text}</blockquote>`
    },
    // Headings
    {
      pattern: /^(#{1,3})\s(.+)$/gm,
      transform: (_, hashes, text) => {
        const size = hashes.length === 1 ? 'text-xl font-bold' : 
                    hashes.length === 2 ? 'text-lg font-semibold' : 
