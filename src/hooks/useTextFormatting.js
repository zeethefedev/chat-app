...
export function useTextFormatting() {
  const insertAtCursor = useCallback((text, insertText, selectionStart, selectionEnd) => {
    return text.substring(0, selectionStart) + insertText + text.substring(selectionEnd);
  }, []);

  const wrapSelection = useCallback((text, wrapper, selectionStart, selectionEnd) => {
  ...
  const handleFormatting = useCallback((text, key, selectionStart, selectionEnd, isShift = false) => {
    const format = getFormatting(key, isShift);
    if (!format) return text;

    if (format.wrapper) {
      return wrapSelection(text, format.wrapper, selectionStart, selectionEnd);
    } else if (format.text) {
