...
function Chatbox() {
  ...
  useEffect(() => {
    ...
    unsubscribeRef.current = subscribeToCollection('messages', (newMessages) => {
      ...
      if (lastMessageRef.current && newMessages.length > 0) {
        ...
        if (lastMessage.id !== lastMessageRef.current && lastMessage.user !== user?.email) {
      ...
      lastMessageRef.current = newMessages.length > 0 ? newMessages[newMessages.length - 1].id : null;
    });
    ...
    `}>
      {isLoading ? (
        <div className="flex-1 flex items-center justify-center">
          <LoadingSpinner size="lg" color={preferences.theme === 'dark' ? 'white' : 'blue'} />
        </div>
      ) : (
        <div 
          ref={containerRef}
